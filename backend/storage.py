...
class ImageStorage:
    ...
    def save_capture(
        ...
        """Save the three images to a new session folder and return metadata."""
        ts = timestamp or datetime.now()
        session_dir = self._new_session_dir(ts)
        prefix = ts.strftime("%Y%m%d_%H%M%S")

        paths = {
            "left": session_dir / f"{prefix}_left.jpg",
            "right": session_dir / f"{prefix}_right.jpg",
            "anaglyph": session_dir / f"{prefix}_anaglyph.jpg",
        }
        arrays = {
            "left": left_bgr,
            "right": right_bgr,
            "anaglyph": anaglyph_bgr,
        }

        for key, path in paths.items():
            ok = cv2.imwrite(str(path), arrays[key], [cv2.IMWRITE_JPEG_QUALITY, JPEG_QUALITY])
            if not ok:
